...
namespace ss
{
  ...
  namespace impl
  {
    ...
    namespace arrays
    {
      ...
      /*
       * Check for ith bit for 0 or 1.
       *
       * assumes: i is a valid offset into the bit array.
       *          If this assumption is not satisfied, the result is undefined.
       *
       * return: non-zero if the ith bit is 1, 0 if the ith bit is zero.
       */
      ...
      {
        // TODO 2:37 PM Sunday, October 26, 2025. Branch unnecessary?
        if((n >> 6) == (N >> 6))
        {
          bits[n >> 6] &= ~blocks::segment_mask(n & 63, N & 63);
          return;
      ...
      /*
       * Set the bit segment [j, j + W) to the given value.
       * Bits are written in offset order from LSB to MSB.
       * Only the bit-offsets [j, j + W) are written.
       *
       * N.B. This would just use W = std::bit_width(j), except that
       *      std::bit_width(0) == 0, as required by the standard (and which makes sense).
       *
       * If the implied offset pair is not contained in `bits`, the resulting `bits` object is undefined.
       */
      constexpr void set_value(auto& bits, std::uint64_t j, std::uint64_t W, std::uint64_t value)
      {
        ...
        for(int i{}; i < W; ++i)
        {
          // bits[b >> 6] =
            ...
            // +
            // (1 - (int)(bool)(value & (1 << i))) * (bits[b >> 6] & ~(1ull << ((i + j) & 63)));

          if(value & (1 << i))
            bits[b >> 6] |= 1ull << ((i + j) & 63);
          else
            bits[b >> 6] &= ~(1ull << ((i + j) & 63));
          ++b;
        }
      ...
      /*
       * Get the value of the bit segment [j, j + W) as a uint64_t.
       * Bits are read in offset from LSB to MSB.
       * Only the bit-offsets [j, j + W) are read.
       *
       * If not { j <= j+W < number of bits in `bits` }, the result is undefined.
       * Else, if W > 64, only the first 64 bits are read.
       */
